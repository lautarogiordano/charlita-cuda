    ...
    for t in range(mcs):
        for i in range(tid, n_agents, stride):
            ...
            if j != i and w[i] > wmin and w[j] > wmin:
                ...
                rand_num = xoroshiro128p_uniform_float64(rng_state, i)
                # Probabilidad de que gane el agente i.
                prob_win = 0.5 + f * (w[j] - w[i]) / (w[i] + w[j])
                # Intercambio
                dw = dw if rand_num <= prob_win else -dw
...
def run_gpu(
    ...
    with cuda.defer_cleanup():
        ...
        for stream, wi, ri, mutex, rng_state in zip(
            streams, wealths, risks, mutexes, rng_states
        ):
...
def get_opponents_cpu(n_agents):
...
def cpu_exchange(n_agents, w, r, wmin, f, mcs):
    ...
    for t in range(mcs):
        opponents = get_opponents_cpu(n_agents)

        for i, j in enumerate(opponents):
